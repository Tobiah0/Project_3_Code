"""
CS 311 Programming Assignment #3
UMass Amherst 2026 Spring
Instructor: Cooper Sigrist
        ...
        for col in range(cols):
            if row == 0 or col == 0 or row == rows - 1 or col == cols - 1:
                ...
                for channel in range(channel_count):
                    sum_of_deltas.append((image[row + 1][col][channel] - image[row - 1][col][channel]) ** 2)
                    sum_of_deltas.append((image[row][col - 1][channel] - image[row][col + 1][channel]) ** 2)

                result[row][col] = sum(sum_of_deltas) ** (1/2)
...
def find_vertical_seam(image: np.ndarray, energy=None):
    ...
    parent = OPT_parent[rows - 1][min_idx]
    path = [min_idx, parent]
